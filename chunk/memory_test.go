...
func TestMemoryChunk(t *testing.T) {
	...
	if c.Checksum() != 0x51cf5c3bc87bacc8 {
		t.Errorf("Checksum() is mismatch: got=0x%016x, want=0x%016x", c.Checksum(), 0x51cf5c3bc87bacc8)
	}
	...
	if n, err := c.Write(in); err != nil {
		...
		t.Errorf("Write() returns wrong n: got=%d, want=%d", n, 8)
	}
	if c.Size() != 13 {
		t.Errorf("Size() is mismatch: got=%d, want=%d", c.Size(), 13)
	}
	if c.Checksum() != 0xf5a8a397b60da2e1 {
		t.Errorf("Checksum() is mismatch: got=0x%016x, want=0x%016x", c.Checksum(), uint64(0xf5a8a397b60da2e1))
	}
